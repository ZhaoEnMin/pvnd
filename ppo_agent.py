...
class InteractionState(object):
    ...
    def __init__(self, ob_space, ac_space, nsteps, gamma, venvs, stochpol, comm):
        ...
        self.stats['tcount'] = 0
        self.empowerments=np.zeros((nenvs, nsteps), np.float32)
        self.emp_per=np.zeros((nenvs, nsteps), np.float32)
        self.empowerments_last = self.empowerments[:, 0, ...].copy()
        self.emp_per_last = self.emp_per[:, 0, ...].copy()
...
#            print('self.ph_ret_int',self.ph_ret_int)
...
#            print('loss,pg_loss ,ent_loss ,vf_loss, self.stochpol.aux_loss,ent_coef',loss,pg_loss ,ent_loss ,vf_loss, self.stochpol.aux_loss,ent_coef)
            #Create optimizer.
...
#        print('ph_buf.shape',len(ph_buf))5
#        print('self.ph_oldnlp',self.ph_oldnlp)
        ...
        #print('ph_buf.shape',len(ph_buf))7
        #print('ph_buf.buf_acs.shape',ph_buf[0][1].shape)32*128
        verbose = True
        if verbose and self.is_log_leader:
            samples = np.prod(self.I.buf_advs.shape)
            logger.info("buffer shape %s, samples_per_mpi=%i, mini_per_mpi=%i, samples=%i, mini=%i " % (str(self.I.buf_advs.shape),
                    samples, samples//self.nminibatches,samples*self.comm_train_size, samples*self.comm_train_size//self.nminibatches))
            logger.info(" "*6 + fmt_row(13, self.loss_names))
        epoch = 0
        start = 0
#        print('self.nepochs',self.nepochs) 4
#        Optimizes on current data for several epochs.
        while epoch < self.nepochs:
            end = start + envsperbatch
            mbenvinds = slice(start, end, None)
#            print('mbenvinds',mbenvinds)0-8 8-16 16-24 24-32
...
#            print('list(fd[self.stochpol.ph_ob[None]].shape)',list(fd[self.stochpol.ph_ob[None]].shape),list(self.ob_space.shape),fd[self.stochpol.ph_ob[None]].shape)[8,129,84,84,4] [84,84,4]()
            ...
            if verbose and self.is_log_leader:
                logger.info("%i:%02i %s" % (epoch, start, fmt_row(13, [lossdict[n] for n in self.loss_names])))
            start += envsperbatch
...
#            print('news',news)
            #print(obs, np.zeros((32,1)), prevrews, news)
            if prevrews is None:
                prevrews=np.zeros((32,1))
            
            self.stochpol.cwp.step(obs, np.zeros((32,1)), prevrews, news)
            ...
            acs, vpreds_int, vpreds_ext, nlps, self.I.mem_state[memsli], ent = self.stochpol.call(dict_obs, news, self.I.mem_state[memsli], update_obs_stats=self.update_ob_stats_every_step)
            #print(acs)
            self.env_step(l, acs)

            #Update buffer with transition.
            #print('k-turn-one-step',self.stochpol.ph_ob_keys) None
            
            for k in self.stochpol.ph_ob_keys:
                self.I.buf_obs[k][sli, t] = dict_obs[k]
                #print(k) None
            #print(dict_obs[k].shape)32*84*84*4 32envs
...
#            print('nlps,t',nlps,t)
#            print('self.I.emp_per[sli,t],self.I.empowerments[sli,t]',self.I.emp_per[sli,t],self.I.empowerments[sli,t])
        ...
        if t == self.nsteps - 1 and not self.disable_policy_update:
            ...
            for l in range(self.I.nlump):
                ...
                self.stochpol.cwp.step(nextobs, acs, rews, nextnews)
                ...
                #with logger.ProfileKV("policy_inference"):
                _, self.I.buf_vpred_int_last[sli], self.I.buf_vpred_ext_last[sli], nlps, _, _ = self.stochpol.call(dict_nextobs, nextnews, self.I.mem_state[memsli], update_obs_stats=False)
                self.I.buf_rews_ext[sli, t] = rews
                for ila in range(32):
                    if nextnews[ila]:
                        self.I.emp_per_last[sli][ila] =nlps[ila]
                        self.I.empowerments_last[sli][ila] =-nlps[ila]
                    else:
                        self.I.emp_per_last[sli][ila] =self.I.emp_per[sli,127][ila]+nlps[ila]
                        self.I.empowerments_last[sli][ila] =self.I.emp_per[sli,127][ila]-2*nlps[ila]
